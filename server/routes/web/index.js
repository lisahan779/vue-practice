module.exports = app => {
  ...
  router.get('/news/init', async (req, res) => {
    ...
    const newsList = newsTitles.map(title => {
      // 随机取上级分类中的数据，打乱数据，slice(0)表示复制一份数组然后打乱；返回的方法： Math.random()返回的是0-1之间的数-0.5根据正负判断随机排序
  ...
  router.get('/news/list', async (req, res) => {
    // 第一种方法有缺选一次性调取的太多数据不均匀前台展示的可能都来自一个分类
    // const parent = await Category.findOne({
    //   name: '新闻'
    // }).populate({
    //   path: 'children',  //关联
    ...
    const cats = await Category.aggregate([
      { $match: { parent: parent._id } },  //类似于where 查询条件找分类为新闻的id
      {
        $lookup: {  //作为外链接
          from: 'articles', //关联集合
          localField: '_id',
          foreignField: 'categories',  //对应文章模型的categories
          as: 'newsList'  //以newlist形式输出
        }
      },
      // 定义需要展示几条 修改newslist的条数
      {
        $addFields: {
    ...
    cats.unshift({
      ...
      newsList: await Article.find().where({
        categories: { $in: subCats }   //$in 定义数据来自cats的id
      }).populate('categories').limit(5).lean() //关联分类查出来数据 
    })
    // 处理前端如果是新闻就显示【新闻】：xxxxxxx  给newList的每一条数据添加了一个对像categoryName：name
  ...
  router.get('/heroes/init', async (req, res) => {
    ...
    for (let cat of rawData) {
      ...
      // 遍历英雄，给每一项英雄加分类，这个分类是对比插入的数据分类关联的数据库分类
  ...
  router.get('/articles/:id', async (req, res) => {
    ...
    data.related = await Article.find().where({  //相关资讯内容
